    ...
    try {
      ...
      const tools = {
        ...
        async st_list_devices() {
          const token = await ensureStAccess(user);
          const apiBase = (process.env.ST_API_BASE || 'https://api.smartthings.com/v1').replace(/\/$/, '');
          const r = await fetch(`${apiBase}/devices`, {
            headers: { 'Authorization': `Bearer ${token}` },
            signal: AbortSignal.timeout(Number(process.env.TIMEOUT_MS || 30000))
          });
          const j = await r.json().catch(() => ({}));
          if (!r.ok) return { ok: false, status: r.status, error: 'smartthings http error', details: j };
          const list = Array.isArray(j?.items) ? j.items : [];
          const norm = list.map(d => ({
        ...
        async st_device_status({ device_id }) {
          if (!device_id) return { ok: false, error: 'device_id required' };
          const token = await ensureStAccess(user);
          const apiBase = (process.env.ST_API_BASE || 'https://api.smartthings.com/v1').replace(/\/$/, '');
          const r = await fetch(`${apiBase}/devices/${encodeURIComponent(device_id)}/status`, {
            headers: { 'Authorization': `Bearer ${token}` },
            signal: AbortSignal.timeout(Number(process.env.TIMEOUT_MS || 30000))
          });
          const j = await r.json().catch(() => ({}));
        ...
        async st_command({ device_id, action, component = 'main', capability, command, args }) {
          if (!device_id) return { ok: false, error: 'device_id required' };
          const token = await ensureStAccess(user);
          const apiBase = (process.env.ST_API_BASE || 'https://api.smartthings.com/v1').replace(/\/$/, '');
          ...
          const payload = {
            commands: [{
              component: String(component || 'main'),
              capability: String(capability),
              command: String(command),
              arguments: Array.isArray(args) ? args : (args != null ? [args] : [])
            }]
          };

          const r = await fetch(`${apiBase}/devices/${encodeURIComponent(device_id)}/commands`, {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
            body: JSON.stringify(payload),
            signal: AbortSignal.timeout(Number(process.env.TIMEOUT_MS || 30000))
        ...
        async tuya_list_devices() {
          const { uid } = await ensureTuyaLinkedUser(user);
          const accessToken = await tuyaEnsureAppToken();
          // tenta /users e fallback /iot-03 (igual ao endpoint HTTP)
          let r = await tuyaSignAndFetch(`/v1.0/users/${encodeURIComponent(uid)}/devices`, { method: 'GET', accessToken });
          if (!(r.status === 200 && r.json?.success === true)) {
            r = await tuyaSignAndFetch(`/v1.0/iot-03/users/${encodeURIComponent(uid)}/devices`, { method: 'GET', accessToken });
          }
          if (r.status !== 200 || r.json?.success !== true) return { ok: false, status: r.status, error: 'tuya http error', details: r.json };
          const items = Array.isArray(r.json.result) ? r.json.result : [];
          const norm = items.map(d => ({
            id: String(d.id || d.uuid || ''),
            name: String(d.name || d.local_key || 'Device'),
        ...
        async tuya_device_status({ device_id }) {
          if (!device_id) return { ok: false, error: 'device_id required' };
          await ensureTuyaLinkedUser(user);
          const token = await tuyaEnsureAppToken();
          const list = await tuyaGetStatus(String(device_id), token);
          const map = Object.fromEntries(list.map(it => [it.code, it.value]));
          const code = await tuyaFindSwitchCode(String(device_id), token);
          const on = code ? !!map[code] : null;
          return { ok: true, on, status: map, code };
        },

        async tuya_command({ device_id, action }) {
          if (!device_id || !action) return { ok: false, error: 'device_id and action required' };
          await ensureTuyaLinkedUser(user);
          const token = await tuyaEnsureAppToken();
